...
object AbbotInGardenScoredRule : Rule {
    ...
    private fun afterTilePlaced(position: Position, state: State): Collection<GameEvent> =
        position.surrounding()
            .filter { state.tileAt(it).hasGarden() }
    ...
    private fun pieceWithOwner(state: State, completedCloisterPosition: Position) =
        state.players.mapNotNull { player -> player.pieceOn(completedCloisterPosition, Abbot)?.let { Pair(player, it) } }
            .firstOrNull()
