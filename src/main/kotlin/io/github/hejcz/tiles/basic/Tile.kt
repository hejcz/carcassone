...
interface Tile : GreenFieldExplorable {
    ...
    fun isValidNeighborFor(neighbor: Tile, direction: Direction): Boolean {
        return exploreCastle(direction).contains(direction) && neighbor.exploreCastle(direction.opposite()).contains(direction.opposite())
            || exploreRoad(direction).contains(direction) && neighbor.exploreRoad(direction.opposite()).contains(direction.opposite())
            || exploreGreenFields(Location(direction)).contains(Location(direction)) && neighbor.exploreGreenFields(Location(direction.opposite())).contains(Location(direction.opposite()))
    }
