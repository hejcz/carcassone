    ...
    describe("Incomplete cloister detector") {
        ...
        it("should detect two incomplete cloisters of different players with 7/8 and 6/8 tiles surrounding tiles respectively") {
            GameScenario(multiPlayer(TileD, TileW, TileV, TileE, TileE, TileB, TileB, TileH, TileK))
                        ...
                        .then(PutTile(Position(-1, 0), Rotation180)).thenReceivedEventShouldBe(SelectPiece)
                        .then(SkipPiece).thenReceivedEventShouldBeOnlyPlaceTile()
                        .then(PutTile(Position(2, 0), Rotation90)).thenReceivedEventShouldBe(SelectPiece)
                        .then(SkipPiece).thenReceivedEventShouldBeOnlyPlaceTile()
                        ...
                        .then(PutPiece(SmallPiece, Monk)).thenReceivedEventShouldBeOnlyPlaceTile()
                        .then(PutTile(Position(-1, -2), NoRotation)).thenReceivedEventShouldBe(SelectPiece)
                        .then(SkipPiece).thenReceivedEventShouldBeOnlyPlaceTile()
                        .then(PutTile(Position(0, -2), Rotation270)).thenReceivedEventShouldBe(SelectPiece)
                .then(SkipPiece)
        ...
        it("should detect two incomplete cloisters of the same player") {
            GameScenario(multiPlayer(TileD, TileW, TileV, TileE, TileB, TileE, TileB, TileH, TileH, TileK))
                        ...
                        .then(PutTile(Position(-1, -2), NoRotation)).thenReceivedEventShouldBe(SelectPiece)
                        .then(SkipPiece).thenReceivedEventShouldBeOnlyPlaceTile()
                        .then(PutTile(Position(2, -2), NoRotation)).thenReceivedEventShouldBe(SelectPiece)
                        .then(SkipPiece).thenReceivedEventShouldBeOnlyPlaceTile()
                        .then(PutTile(Position(0, -2), Rotation270)).thenReceivedEventShouldBe(SelectPiece)
                .then(SkipPiece)
